    ...
    public boolean visit(MethodInvocation node) {
        ...
        while (!handled && (i < SaveStateCodeGenerator.saveStateNodeTypes.length)) {
            if ((node.getName().toString().equals(
                    SaveStateCodeGenerator.saveStateNodeTypes[i].getProperty(ViewProperties.ViewStateGetMethod)))) {
                identifySavedView(node);
                handled = true;
            } else if (node.getName().toString().equals(
                    SaveStateCodeGenerator.saveStateNodeTypes[i].getProperty(ViewProperties.ViewStateSetMethod))) {
        ...
        if (expression instanceof SimpleName) {
            ...
            if (javaElement != null) {
                try {
                    ...
                    AndmoreLogger.warn(CodeUtilsActivator.PLUGIN_ID,
                            "Unable to identify if " + binding.getName() + " is a subclass of android.view.View", e);
                }
            }
        }
    }
