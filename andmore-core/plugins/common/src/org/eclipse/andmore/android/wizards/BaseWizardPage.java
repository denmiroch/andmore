...
public abstract class BaseWizardPage extends WizardPage implements Listener {
     ...
     * 
     * @param block
     *            Block used to render the screen.
     * @param title
     *            Window title
     * @param description
     *            Window description
     ...
     *            Window description
     * @param contextHelpID
     *            The context help id used to this page
     */
    ...
    public void createControl(Composite parent) {
        ...
        if (block != null) {
            block.setShell(getShell());
            Composite blockComposite = block.createContent(getComposite(parent));
            if (helpID != null) {
    ...
    protected Composite getComposite(Composite parent) {
        if (composite == null) {
            composite = WidgetsFactory.createComposite(parent);
        }
        return composite;
    }
    ...
    protected GridLayout getLayout() {
        if (layout == null) {
            layout = WidgetsFactory.createGridLayout();
        }
        return layout;
    }
    ...
    public boolean canFlipToNextPage() {
        ...
        return block.canFlipToNextPage() && (getNextPage() != null);
    }
}
