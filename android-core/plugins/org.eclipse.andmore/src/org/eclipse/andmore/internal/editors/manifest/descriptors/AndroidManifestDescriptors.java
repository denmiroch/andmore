...
public final class AndroidManifestDescriptors implements IDescriptorProvider {
    ...
    private static final String ANDROID_MANIFEST_STYLEABLE = AttrsXmlParser.ANDROID_MANIFEST_STYLEABLE;
    ...
    public AndroidManifestDescriptors() {
        ...
        MANIFEST_ELEMENT = createElement(MANIFEST_NODE_NAME, // xml name
                new ElementDescriptor[] { APPLICATION_ELEMENT, INTRUMENTATION_ELEMENT, PERMISSION_ELEMENT,
                        USES_PERMISSION_ELEMENT, PERMISSION_GROUP_ELEMENT, PERMISSION_TREE_ELEMENT, USES_SDK_ELEMENT, },
                Mandatory.MANDATORY);

        // The "package" attribute is treated differently as it doesn't have the standard
        // Android XML namespace.
        PACKAGE_ATTR_DESC = new PackageAttributeDescriptor(PACKAGE_ATTR, null /* nsUri */,
                new AttributeInfo(PACKAGE_ATTR, Format.REFERENCE_SET))
                        .setTooltip("This attribute gives a unique name for the package, using a Java-style "
    ...
    public synchronized void updateDescriptors(Map<String, DeclareStyleableInfo> manifestMap) {
        ...
        overrideClassName(overrides, "application", //$NON-NLS-1$
                SdkConstants.CLASS_APPLICATION, false /*mandatory*/);
        ...
        overrideClassName(overrides, "instrumentation", SdkConstants.CLASS_INSTRUMENTATION);
        ...
        inflateElement(manifestMap, overrides, required, elementDescs, MANIFEST_ELEMENT, "AndroidManifest"); //$NON-NLS-1$
     ...
     */
    private static void overrideClassName(Map<String, ITextAttributeCreator> overrides, String elementName,
     ...
     */
    private static void overrideClassName(Map<String, ITextAttributeCreator> overrides, String elementName,
            final String className, final boolean mandatory) {
        ...
        overrides.put(elementName, new ITextAttributeCreator() {
            ...
            public TextAttributeDescriptor create(String xmlName, String nsUri, IAttributeInfo attrInfo) {
                ...
                if (SdkConstants.CLASS_ACTIVITY.equals(className)) {
                    return new ClassAttributeDescriptor(className, PostActivityCreationAction.getAction(), xmlName,
                            nsUri, attrInfo, mandatory, true /*defaultToProjectOnly*/);
                } else if (SdkConstants.CLASS_BROADCASTRECEIVER.equals(className)) {
                    return new ClassAttributeDescriptor(className, PostReceiverCreationAction.getAction(), xmlName,
                            nsUri, attrInfo, mandatory, true /*defaultToProjectOnly*/);
                } else if (SdkConstants.CLASS_INSTRUMENTATION.equals(className)) {
                    return new ClassAttributeDescriptor(className, null, // no post action
                            xmlName, nsUri, attrInfo, mandatory, false /*defaultToProjectOnly*/);
                } else {
     ...
     */
    private ElementDescriptor createElement(String xmlName, ElementDescriptor[] childrenElements, Mandatory mandatory) {
        // Creates an element with no attribute overrides.
        ...
        ElementDescriptor element = new ManifestElementDescriptor(xmlName, uiName, null, sdkUrl, null, childrenElements,
                mandatory);
     ...
     */
    private void inflateElement(Map<String, DeclareStyleableInfo> styleMap,
            Map<String, ITextAttributeCreator> overrides, Set<String> requiredAttributes,
            HashMap<String, ElementDescriptor> existingElementDescs, ElementDescriptor elemDesc, String styleName) {
        ...
        if (style != null) {
            ...
            DescriptorsUtils.appendAttributes(attrDescs, elemDesc.getXmlLocalName(), SdkConstants.NS_RESOURCES,
                    style.getAttributes(), requiredAttributes, overrides);
        ...
        for (Entry<String, DeclareStyleableInfo> entry : styleMap.entrySet()) {
            ...
            if (isParent) {
                ...
                inflateElement(styleMap, overrides, requiredAttributes, existingElementDescs, child, childStyleName);
            }
        }
